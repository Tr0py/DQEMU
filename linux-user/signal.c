...
extern int offload_segfault_handler(int host_signum, siginfo_t *pinfo, void *puc);
extern int client_segfault_handler(int host_signum, siginfo_t *pinfo, void *puc);
                                ...
                                void *puc)
{
	...
	if ((host_signum == SIGSEGV) && (info->si_code == SEGV_ACCERR))
	{
		/*if (offload_mode == 1)
		{
			if (server_segfault_handler(host_signum, info, puc))
			{
				return;
			}
		}
		...
		{
			if (client_segfault_handler(host_signum, info, puc))
			{
				return;
			}
		}*/
		offload_segfault_handler(host_signum, info, puc);
